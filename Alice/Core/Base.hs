...
defs = IM.fromList [(-1, eq),(-2, less), (-4, fun),(-5,app),       -- initial definitions
...
getLink :: [String] -> VM (Set.Set String) -- get the section identifiers grouped
getLink ln = do grps <- askGS gsGrup;      -- by a group identifier
                return $ Set.unions $ map (\l -> M.findWithDefault (Set.singleton l) l grps) ln

addGroup [] = return () -- add a group identifier
addGroup [nm] = rlog0 $ "Warning: empty group: " ++ show nm
addGroup (nm:ps) = do getLink ps >>= \ln -> updateGS (\st -> st {gsGrup = M.insert nm ln $ gsGrup st})

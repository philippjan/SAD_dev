...
fillDef cx
  ...
  where
    ...
    fill _ _ _ _ _ t | isThesis t
      = thesis >>= return . cnForm
    fill _ _ fc _ _ v | isVar v                                           -- for variables, no definitions must be checked
      = do  uin <- askRSIB IBinfo True                           -- see if user has disables info collection
            nct <- cnRaise cx fc
...
setInfo t = do cnt <- asks vsCtxt; let loc = takeWhile cnLowL cnt; lev = fromMaybe [] $ typings loc t

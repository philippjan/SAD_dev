...
getEv form = filter (isJust . find (twins ThisT) . ltArgs) . filter isLtrl . deAnd $ form
...
closeEvid dfs df@DE{dfEvid = evs} = df { dfEvid = newEvid }
  where
    ...
    evids t@Trm {trId = n} = let def = fromJust $ IM.lookup n dfs
...
extrFun :: (Formula -> Formula) -> [Formula] -> Formula -> ReaderT (Int, DT.DisTree Eval) [] Eval
...
extrFun c gs f = dive c gs f
  where
    dive c gs (Tag DCD (Imp _ g)) = (lift $ deConj g) >>= dive c gs
    dive c gs (Imp f g) = dive c (f:gs) g
    dive c gs (All _ f) = freshV (dive c gs) f
    dive c gs (And f g) = dive (c . And f) gs g
    dive c gs (Tag DEV (All _ (Iff Trm {trArgs = [tr,_]} f))) = let nf = c $ instWith ThisT f in return $ EV tr nf nf gs
    dive c gs (Tag DEV f@Trm{ trName = "=", trArgs = [tr,vl]} ) = let nf = c f {trArgs = [ThisT, vl] } in return $ EV tr nf nf gs
    dive c gs (Exi x (And (Tag DEF f) (Tag DEV Trm {trName = "=", trArgs = [tr, Ind n]}))) | n == 0 = extractEv c gs $ dec $ instWith tr f
    dive c gs (Exi x f) = dive (c . zExi x) gs $ inst x f
    dive _ _ _ = mzero
...
extrSet c gs (All _ (Iff g@Trm{trArgs = [_,t]} f ))
  | isElem g = do (n, dt) <- ask; let nm = '?':show n; nf = elemsimp dt $ strip $ inst nm f
...
elemsimp dt = dive
  where
    ...
    simp f = do ev <- DT.lookup f dt >>= single; guard (null $ evCond ev); sb <- match (evTerm ev) f; return $ sb $ evNeg ev

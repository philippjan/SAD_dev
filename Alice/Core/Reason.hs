...
lookFor l trm = let tId = trId (predSymb l); negl = albet $ Not l
...
unfoldA sg fr =
  ...
  where
    ...
    getExp Trm {trName = "=", trArgs = [l,r]}
      = liftM3 (\a b c -> a ++ b ++ c) (deq l r) (deq r l) (setDeq l r) -- we combine definitional information for l, for r and if
    ...
    deq f g =
     do dfs <- asks dfs
        let ls = maybeToList $
             do mId <- mbId f; df <- IM.lookup (mId) dfs; guard (sg || dfSign df) -- only unfold a definitions or a sigext in a positive position
                sb <- match (dfTerm df) f; let dq = replace (Tag DMK g) ThisT $ sb $ dfForm df -- substitute the (marked) term
                guard (not . isTop $ dq); return dq
        unfGuard ufl $ unless (null ls) (lift $ W.tell 1) >> return ls -- increase the counter by 1 and return what we got
    ...
    setfun_deq t =
      do dt <- asks evs
       ...
       where
         findev ev = do sb <- match (evTerm ev) t; guard (all rapid $ map sb $ evCond ev)
                        return $ replace (Tag DMK t) ThisT $ sb $ if sg then evPos $ ev else evNeg $ ev

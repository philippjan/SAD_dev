...
vLoop st@VS {vsMotv = mot, vsRuls = rls, vsThes = ths, vsBran = brn, vsCtxt = cnt,
             vsText = TB bl@(Block fr pr sg dv _ _ la _ _ _) : bs, vsEval = evs } = local (const st) $
  do  incRSCI CIsect ; tfn <- askRSIS ISfile "" ; whenIB IBPsct False $
      ...
      --when (sg == Declare || sg == Defn || sg == Sign) $ rlog0 $ DT.showTree evs

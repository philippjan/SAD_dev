...
alias = do  nulText ; (f, g) <- narrow $ prd -|- ntn
            getS >>= newExpr f (renull g) False; return ()
  where
...
sig_ntn = do  (n, u) <- old_ntn is; (q, f) <- anotion -|- nmn
  ...
  where
    nmn = art >> wordOf ["notion","constant"] >> return (id,Top)
    trm Trm {trName = "=", trArgs = [_,t]} = t ; trm t = t




params Trm {trId = tId, trArgs = [_,t]} | tId == elmId = trArgs t
params t | isTrm t =  tail $ trArgs t
...
prdvars f d | not flt   = nextfail $  "compound expression: " ++ show f
  ...
  where
    ...
    pvs ls (Var {trName = v@('h':_)}: vs)    = notElem v ls && pvs (v:ls) vs
    pvs ls (Var {trName = v@('x':_)}: vs)    = notElem v ls && pvs (v:ls) vs
    pvs _ []                       = True
    pvs _ _                        = False

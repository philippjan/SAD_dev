...
proof.
    Let y << (I +' J).
    (1) Take elements k,l such that k << I and l << J such that x = k + l (by DefSum).
    (2) Take elements m,n such that m << I and n << J such that y = m + n (by DefSum).
    k + m belongs to I and l + n belongs to J.
    x + y .= (k + m) + (l + n) (by 1, 2, AddComm,AddAsso,AddBubble).
    Therefore the thesis (by DefSum).
    end.
For every element z (z * x) << (I +' J).
proof.
    Let z be an element. (1) Take k << I and l << J such that x = k + l (by DefSum).
...
Signature NatLess.  Let i,j be natural numbers. i -<- j is a relation.
...
proof.
    Let y << <c>.
    (1) Take an element u such that c * u = x (by DefPrIdeal).
    (2) Take an element v such that c * v = y (by DefPrideal).
    x + y .= c * (u + v) (by 1, 2, AMDistr1).
    Therefore the thesis (by DefPrIdeal).
    end.
forall z z * x << <c>.
proof.
    Let z be an element.
...
proof.
    ...
    proof.
        Take k << <a> and l << <b> such that u = k + l (by DefSum).
        Take elements x,y such that (k = a * x and l = b * y) (by DefPrIdeal).
        Hence the thesis.
        end.
